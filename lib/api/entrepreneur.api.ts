...
export async function updateIdeaApi(data: any, startupId: string) {
    const token = useAuthStore.getState().token;
    ...
    const res = await fetch(`${API_URL}/entrepreneur/ideas/${startupId}`, {
        method: 'PATCH',
        headers: { Authorization: `Bearer ${token}`, },
        body: formData,
    })
    ...
    if (res.status === 200) {
        return Promise.resolve({ success: true, data: await res.json() }) as Promise<{ 
                    success: boolean;
                    data: {
                        idea: StartupResponse; 
                    }}>;
    } else {
        const errorData = await res.json()
        throw new Error(errorData.message || 'Failed to update Ideas')
    }
}

export async function deleteIdeaApi(startupId: string) {
    const token = useAuthStore.getState().token;
